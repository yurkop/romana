...
//-------------------------------------
UShort_t ClassToBuf(const char *name, const char *varname, char *var,
                    char *buf) {
...
void BufToClass(const char *name, const char *varname, char *var, char *buf,
                int size) {
  ...
  while (sz < size) {
    ...
    if (!strcmp(clname, name) &&
        !strcmp(vname, varname)) { // the same class & var
      TIter nextd(lst);
      while ((dm = (TDataMember *)nextd())) {
        if (!strcmp(dm->GetName(), memname)) {
          ...
          if (s1 && s2) {
            ...
            // cout << "dm: " << dm << " " << str << " " << len << " " <<
            // dm->GetName() << " " << dm->GetTitle() << endl; exit(-1);
            if (!strcmp(str, memname)) {
              cout << "dm2: " << dm << " " << str << " " << len << " "
                   << dm->GetName() << " " << dm->GetTitle() << endl;
              // exit(-1);
              break;
            }
          }
        }
        // cout << "dm: " << dm << " " << dm->GetName() << " " << dm->GetTitle()
        // << endl;
      }
      // cout << "dm2: " << dm << endl;
      // if (dm) {
      //  	cout << "dm: " << dm << " " << dm->GetName() << " " <<
      //  dm->GetTitle() << endl;
      // }
  ...
  if (p2) {
    ...
    while ((dm = (TDataMember *)nextd())) {
      if (dm->GetDataType() == 0 && TString(dm->GetName()).Contains("h_")) {
        // cout << "member: " << dm->GetName() << " " << dm->GetDataType() << "
        // " << dm->GetOffset() << " " << (void*)popt+dm->GetOffset() << " " <<
        // &opt << " " << &(opt.h_time) << endl;

...
public:
  ...
  Float_t Time; // exact time relative to pulse start (from 1st deriv), also
                // relative to event start, in samples
...
void PulseClass::Ana(int what) {
  ...
  case 2:
    ...
    if (alp[iter].Tstamp == Tstamp64) {
      ...
      // 	 << ort[iter].Tstamp << " " << Tstamp64 << " " << sData.size()
      // << endl;
      ++iter;
    }
    break;
  case 3:
    cout << "Pulse: " << (int)Chan << " " << Counter << " " << Tstamp64 << " "
         << sData.size() << endl;
...
void PulseClass::SavePulse(const char *fname) {
  ...
  *ofs << iter << " " << sData.size() << " " << ort[iter].Tstamp << " "
       << ort[iter].EE << " " << alp[iter].Tstamp << " " << alp[iter].EE
       << endl;
...
void decode34(char *buf, Long64_t &idx1, Long64_t size) {
  ...
  peak_type *ipk =
      &dummy_peak; // pointer to the current peak in the current pulse;
  ...
  while (idx1 < size) {
    ...
    // if (frmt && Blist->empty()) {
    //   ++errors[0];
    //   //cout << "dec34: bad buf start: " << idx1 << " " << (int) ch << " " <<
    //   frmt << endl; idx1+=8; continue;
    // }
    // else if (ch==255) {
    //   //start signal
    //   idx1+=8;

...
int Command32(UChar_t cmd, UChar_t ch, UChar_t type, int par, int tmout,
              int delay, int ans);
...
int Command32(UChar_t cmd, UChar_t ch, UChar_t type, int par, int tmout,
  ...
  printf("Cmd32: %d %d %d %d %d %d\n", (int)cmd, (int)ch, (int)type, par, tmout,
         ans);
  ...
  r = cyusb_bulk_transfer(cy_handle, 0x01, buf_out, len_out, &transferred,
                          tmout);
  ...
  if (ans && cmd != 7 && cmd != 12) { // 7 и 12: сброс USB и Тест
    ...
    r = cyusb_bulk_transfer(cy_handle, 0x81, buf_in, len_in, &transferred,
                            tmout);

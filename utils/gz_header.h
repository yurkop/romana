...
public:
  ...
  Int_t durWr[MAX_CH +
              ADDCH]; // total length N = 1…32763 (internally - multiple of 4)
  Int_t kderiv[MAX_CH + ADDCH];    // K = 0...1023; K=0 - trigger on the signal;
                                   // k!=0 - on derivative
...
public:
  ...
  void GetPar(const char *name, Int_t module, Int_t i, Int_t &par, Int_t &min,
              Int_t &max) {};

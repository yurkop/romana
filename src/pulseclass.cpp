...
void PulseClass::PeakAna33(bool onlyT) {
  ...
  // prnt("ss d l d l d
  // ds;",BGRN,"P33:",BGRN,Chan,Tstamp64,Pos,sData.size(),opt.sTg[Chan],onlyT,RST);
  ...
  for (int j = T1; j <= T2; j++) {
    ...
    if (dif < 0) {
      ...
      if (opt.Mr[Chan] > 0 && j - T1 > opt.Mr[Chan])
    ...
    if (crs->use_2nd_deriv[Chan]) { // use 2nd deriv
      ...
      for (int j = T1; j <= T2; j++) {
        if (j < kk + 1)
          continue;
        Float_t dif2 =
            sData[j] - sData[j - kk] - sData[j - 1] + sData[j - kk - 1];
        // if (dif2>0) {
        Time += dif2 * j;
        Area2 += dif2;
        //}
  ...
  if (B2 <= B1)
    B2 = B1; // base can NOT be zero if B2==B1
  if (P2 <= P1)
    P2 = P1;
  if (W2 <= W1)
    W2 = W1;
  ...
  if (nn) {
    ...
    ++crs->errors[ER_AREA];
    // cout << "zero Area: " << this->Tstamp64 << " " << Pos << " " << P1 << " "
    // << P2 << endl;
  }
  ...
  // prnt("ss d l d d
  // fs;",BYEL,"Peakana33:",Chan,Tstamp64,cpar.Pre[Chan],Pos,Time,RST);
...
void PulseClass::PoleZero(int Pz) {
  ...
  for (UInt_t i = 1; i < sData.size(); i++) {
    ...
    // if (i>2000) {
    Float_t dd =
        sData[i - 1] - x1 * cor; // dd=x'[k-1]-x[k-1]*cor
                                 //  if (Tstamp64==2778120) {
                                 //  	cout << "pz: " << i << " " << sData[i]
                                 //  << " " << sData[i-1] << " " << x0
                                 //  	     << " " << x1 << " " << dd << endl;
...
void EventClass::PrintEvent(bool pls) {
  ...
  if (pls)
    for (UInt_t i = 0; i < pulses.size(); i++) {
      printf("  %3d %lld %10.2f\n", pulses[i].Chan, pulses[i].Tstamp64,
             pulses[i].Time);
    }
}

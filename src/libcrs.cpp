...
int CRS::Init_device() {
  ...
  case 3: // crs-16 or crs-2
    ...
    if (opt.ver_po == 1) { // версия ПО=1
      ...
      // для версии ПО=9 и выше задаем маску - оказалась не нужна!
      // if (opt.ver_po >= 9) {
      //   cpar.chmask = 0x1F;
      // }
    }
  ...
  while (idx1 < b_end[ibuf]) {
    ...
    case 0:
      ...
      if (ipls.ptype == 0) { // analyze old pulse
        ...
        PulseAna(ipls);
        // if (ipls.Tstamp64<1e6)
        // prnt("ss d f ls;", BMAG, "P2:", ipls.Chan, ipls.Time, ipls.Tstamp64,
            //  RST);
        Event_Insert_Pulse(Blist, &ipls);
...
void CRS::Make_Events(std::list<eventlist>::iterator BB) {
  ...
  if (!Levents.empty() && !BB->empty() && (spn == 255)) {
    ...
    while (it != BB->end() && it->Tstmp <= T_last) {
      ...
      for (auto &pls : it->pulses) {
        // восстанавливаем время импульса, т.к. оно было привязано к Tstmp
        // события
        pls.Time -= pls.Tstamp64 - it->Tstmp;
        Event_Insert_Pulse(&Levents, &pls);
      }
      it = BB->erase(it);
    }
  }

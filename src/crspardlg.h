...
public:
  ...
  // int jtrig;
  // bool notbuilt;
  // int pmax; //максимальный канал (вместо MAX_CH), который записан в
  // параметрах
...
protected:
  ...
  TGHorizontalFrame *hparl[3][MAX_CHTP]; // горизонтальные фреймы в chanpar;
                                         // должно заменить cframe
...
public:
  ...
  void DoMap(TGFrame *f, void *d, P_Def t, int all, UInt_t cmd = 0,
             void *d2 = 0, UShort_t off = 0, UChar_t step = 1);
  ...
  void Check_opt(TGHorizontalFrame *hfr1, int width, void *x1, const char *tip1,
                 UInt_t cmd1, const char *cname = "");

  void Num_opt(TGHorizontalFrame *hfr1, int width, void *x1, void *x1a,
               const char *tip1, TGNumberFormat::EStyle style1, double min1,
               double max1, UInt_t cmd1, TGLayoutHints *Lay);

  void AddLine_opt(TGCompositeFrame *frame, int width, void *x1, void *x2,
                   const char *tip1, const char *tip2, const char *label,
                   TGNumberFormat::EStyle style1, TGNumberFormat::EStyle style2,
                   // TGNumberFormat::EAttribute attr,
                   double min1 = 0, double max1 = 0, double min2 = 0,
                   double max2 = 0, UInt_t cmd1 = 0, UInt_t cmd2 = 0,
  ...
  void AddLine_1opt(TGCompositeFrame *frame, int width, void *x1, void *x1a,
                    ...
                    TGNumberFormat::EStyle style1, double min1 = 0,
                    double max1 = 0, UInt_t cmd1 = 0, TGLayoutHints *Lay1 = 0);

  void AddLine_txt(TGCompositeFrame *frame, int width, void *x1,
                   const char *tip1, const char *label, UInt_t cmd1 = 0,
                   TGLayoutHints *Lay1 = 0);
...
public:
  ...
  void AddColumn(int jj, int kk, int ii, P_Def pdef, int wd, int dfile, int dev,
                 double min, double max, const char *pname, void *apar = 0,
                 void *apar2 = 0, UInt_t cmd = 0, int s2 = 0);
  // void AddHeader();
  ...
  void AddChkPar(int kk, int wd, int all, int dfile, TGHorizontalFrame *hfr,
                 void *apar, UShort_t off = 0, UInt_t cmd = 1, void *apar2 = 0,
                 UChar_t step = 1);
  void AddNumPar(int i, int kk, int wd, int all, int dfile, int dev, P_Def pdef,
                 double min, double max, TGHorizontalFrame *hfr,
                 const char *name, void *apar, UShort_t off = 0, UInt_t cmd = 1,
                 void *apar2 = 0);
  void AddStatDaq(int jj, int kk, int wd, void *apar, TGHorizontalFrame *hfr);
...
public:
  ...
  void Add_prof_num(TGHorizontalFrame *hfr1, void *nnn, Int_t max, P_Def pp,
                    UInt_t cmd, const char *tip);
...
#ifdef YUMO
  void Add_yumo(TGGroupFrame *frame, Mdef *md1, Mdef *md2, Mdef *md3);
#endif
  //, Hdef* hd,

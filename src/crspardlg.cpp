...
extern Pixel_t
    fCol[7]; // = [1..7] {fYellow,fGreen,fRed,fCyan,fOrng,fBlue,fRed10};
...
const char *ttip_type = "Channel type:\nOther - dummy type\nCopy - copy from "
                        ...
                        "change parameter(s), then change to new type";

vector<const char *> ptip = {
    "Channel number", "On/Off", "Select", ttip_type, "Inversion",
    "AC coupling\nFor CRS-128 grouped by 4 channels",
    "Send/don't send pulse data (waveforms)", "Send/don't send DSP data",
    ...
    "Hardware smoothing. It is recommended to set it to the power of 2 to "
    "avoid reduction of amplitude.",
    "Hardware delay (in samples)",
    "Dead time - no new trigger on the current channel within dead time from "
    "the old trigger",
    ...
    "channels in the event, the earliest is used",
    "Master/slave channel:\nEvents containing only slave channels are "
    ...
    "  - use hardware pulse analysis for other parameters (Area, Base, Width, "
    "Height)",
    ...
    "Energy calibration type: 0 - no calibration; 1 - linear; 2 - parabola; 3 "
    "- spline",
    "Energy calibration 0: E0+E1*x", "Energy calibration 1: E0+E1*x",
    "Energy calibration 2: E0+E1*x+E2*x*x",
    "Pole-Zero correction\nPositive: correct for exponential decay in "
    ...
    "Software smoothing. If negative - imitates hS (data are truncated to "
    "integer etc)",
    ...
    "Software trigger threshold", "Software trigger lower threshold",
    "Analysis method:\n0 - standard;\n1 - area from 1st derivative between T1 "
    "and T2; no base subtraction\n2 - base slope subtraction (for HPGe)\n3 - "
    "base slope2 instead of slope1 (using W1 & W2) + slope2 subtraction (for "
    ...
    //"CFD fraction x10",
    "Width window start (included)", "Width window end (included)",
    "Ntof flight path (in meters) for Ntof-Energy conversion",
    "Pulse rate (software)", "Pulse rate (hardware)", "Number of bad pulses"};
...
const char *tip_dec = "Mask for decoder format (type 81 or higher)\n" mask0
                      " (spaces are ignored)\n"
                      "\n"
...
ParDlg::ParDlg(const TGWindow *p, UInt_t w, UInt_t h)
  ...
  for (int i = 0; i < MAX_TP + 3; i++) {
    cc = TColor::GetColor(
        TColor::RGB2Pixel(CP::RGB[i][0], CP::RGB[i][1], CP::RGB[i][2]));
...
void ParDlg::DoOpen() {
  ...
  const char *dnd_types[] = {"all files", "*",
                             // ".raw files",     "*.raw",
                             // ".dec files",     "*.dec",
                             // ".root files",     "*.root",
                             // ".gz files",     "*.gz",
...
void ParDlg::DoCombo() {
  ...
  // prnt("ss d d d d d
  // ds;",BRED,"comb:",id,sel,index,nline,pp->off,pp->all,RST);

  if (pp->all == 0) {        // normal channels
    if (sel != MAX_TP + 2) { // sel!=Copy -> normal sel
      SetCombo(*pp, index, sel);
      if (old_type !=
          MAX_TP + 3) { // if old type is not swap, then copy one line
...
void ParDlg::CopyParLine(int sel, int index, int line) {
  ...
  if (sel < 0) { // inverse copy - from current ch to group
    ...
    for (int j = 0; j < nfld; j++) {
      Pmap *pp = &Plist[j];
      // prnt("ss d d d d x x
      // ds;",BYEL,"CPL:",j,sel,index,line,pp->data,&cpar.RD,pp->cmd &
      // 0x800,RST);
...
void ParDlg::DoColor(Pmap *pp, Float_t val) {
  ...
  if (col) {
    ...
    case p_fnum2: {
      ...
      if (te->IsEnabled()) {
        bool vv = (val != 0);
        if (te->GetToolTip()->GetText()->Contains("rebin",
                                                  TString::kIgnoreCase)) {
          vv = val > 1;
        }
        if (vv) {
...
void ParDlg::DaqDisable() {
  ...
  for (UInt_t i=0;i<Plist.size();i++) {
    ...
    if (pp->cmd & 0x100) {
      ...
      // if (pp->type == p_chk)
      //  	col = BRED;
      // prnt("ss d d d ds;",col,"daqdis:
",i,pp->type,pp->cmd,(int)wg->IsEnabled(),RST); EnableField(i,false);
      // prnt("ss d d d ds;",col,"daqdis2:
",i,pp->type,pp->cmd,(int)wg->IsEnabled(),RST);
    }
  }
}
...
void ParDlg::Num_opt(TGHorizontalFrame *hfr1, int width, void *x1, void *x1a,
  ...
  if (style1 != k_lab) {
    ...
    if (width > 0) {
      fNum1 =
          new TGNumberEntry(hfr1, 0, 0, id, style1,
      ...
      fNum2 = new TGNumberEntryField(hfr1, id, 0, style1,
                                     TGNumberFormat::kNEAAnyNumber, limits,
                                     min1, max1);
...
void ParDlg::AddLine_opt(TGCompositeFrame *frame, int width, void *x1, void *x2,
                         const char *tip1, const char *tip2, const char *label,
                         TGNumberFormat::EStyle style1,
                         TGNumberFormat::EStyle style2, double min1,
                         double max1, double min2, double max2, UInt_t cmd1,
                         UInt_t cmd2, TGLayoutHints *Lay1,
                         TGLayoutHints *Lay2) {
...
void ParDlg::AddLine_1opt(TGCompositeFrame *frame, int width, void *x1,
                          ...
                          TGNumberFormat::EStyle style1, double min1,
                          double max1, UInt_t cmd1, TGLayoutHints *Lay1) {
  if (!Lay1)
    Lay1 = LayLT4;
...
void ParDlg::AddLine_txt(TGCompositeFrame *frame, int width, void *x1,
                         const char *tip1, const char *label, UInt_t cmd1,
                         TGLayoutHints *Lay1) {
  if (!Lay1)
    Lay1 = LayLT4;
...
//------ TrigFrame -------
TrigFrame::TrigFrame(TGGroupFrame *p, int opt) : TGHorizontalFrame(p) {
  ...
  TGLayoutHints *LayLC1 =
      new TGLayoutHints(kLHintsLeft | kLHintsCenterY, 1, 0, 1, 1);
...
ParParDlg::ParParDlg(const TGWindow *p, UInt_t w, UInt_t h) : ParDlg(p, w, h) {
  // soft_list.clear();
  // hard_list.clear();
  ...
  TGVerticalFrame *fV1 =
      new TGVerticalFrame(hF1, 1, 1, kFixedWidth | kSunkenFrame);
...
void ParParDlg::AddChk(TGGroupFrame *frame, const char *txt, Bool_t *opt_chk,
  ...
  TGNumberEntry *fNum1 = new TGNumberEntry(
      hframe1, *compr, 2, id, k_int, TGNumberFormat::kNEAAnyNumber,
      TGNumberFormat::kNELLimitMinMax, 0, 9);
  ...
  if (rflag) {
    ...
    fchk2->SetToolTipText("input raw: Checked - write processed events;
    unchecked - write direct raw stream\ninput dec: Checked - reanalyse .dec
    file with new coincidence conditions");
    //fchk2->SetName(txt);
...
int ParParDlg::AddOpt(TGCompositeFrame *frame) {
  ...
  AddLine_opt(fF6, ww, &opt.Nchan, &opt.nthreads, tip1, tip2, label, k_int,
              k_int, 1, MAX_CH, 1, CRS::MAXTHREADS, 0x100, 0x100);
  ...
  AddLine_opt(fF6, ww, &opt.usb_size, &opt.rbuf_size, tip1, tip2, label, k_int,
              k_int, 1, 2048, 1, 64000, 0x100, 0x100 | (1 << 4));

  tip1 = "Maximal size of the event list:\nNumber of events available for "
         "viewing in the Events Tab";
  tip2 = "Event lag:\nMaximal expected number of lagged events (see "
         "Errors/Event lag exceeded\nMust be at least 1)";
  label = "Event_list size / Event lag";
  AddLine_opt(fF6, ww, &opt.ev_max, &opt.ev_min, tip1, tip2, label, k_int,
              k_int, 1, 9999999, 1, 1000000);
  ...
  tip2 = "[CRS-8/16] Soft start (imitator) period (sec):\n"
         ...
         "For CRS-8/128:  1: 0.08, 2: 0.17, 3: 0.33, 4: 0.67, 5: 1.34, 6: "
         "2.68, 7: 5.36, 8: 10.7\n"
...
int ParParDlg::AddNtof(TGCompositeFrame *frame) {
  ...
  AddLine_opt(fF6, ww, &opt.TofZero, &opt.start_ch, tip1, tip2, label, k_r0,
              k_int, -1e9, 1e9, 0, 255, 0x400, 0x400);
...
int ParParDlg::AddLogic(TGCompositeFrame *frame) {
  ...
  tip2 = "Veto window (in samples): \nsubsequent pulses from the same channel "
         "coming within this window are ignored";
  label = "Coincidence (smp), veto (smp)";
  AddLine_opt(fF6, ww, &opt.tgate, &opt.tveto, tip1, tip2, label, k_int, k_int,
              0, 10000, 0, 1000);
  ...
  tip1 = "";
  tip2 = "Main trigegr condition (cut).\nThis condition is used for selecting "
         "events which are written as decoded events\nSee \"Plots\" for making "
         "conditions\nUse this cut number as a main trigger condition.\nIf set "
         "to zero - write all events.";
  label = "Main trigger";
  AddLine_opt(fF6, ww, NULL, &opt.maintrig, tip1, tip2, label, k_lab, k_int, 0,
              0, 0, MAXCUTS);
...
int ParParDlg::AddExpert(TGCompositeFrame *frame) {
  ...
  tip1 = "Hex bitwise raw data transfer mask:\n"
         "default value: FFFF\n\n"
...
HistParDlg::HistParDlg(const TGWindow *p, UInt_t w, UInt_t h)
  ...
  fcont1 = new TGCompositeFrame(fCanvas1->GetViewPort(), 10, 10);
  fCanvas1->SetContainer(fcont1);
...
void HistParDlg::AddHist(TGCompositeFrame *frame2) {
  ...
  // cout << "AddHist: " << endl;
  TGGroupFrame *f1 =
      new TGGroupFrame(frame2, "1D Histograms", kHorizontalFrame);
  ...
  for (auto it = hcl->Mlist.begin(); it != hcl->Mlist.end(); ++it) {
    ...
    if (it->hnum < 50) { // 1d hist
      ...
      if (hcl->md_pulse) {
        ...
        DoMap(chk_hist2, &hcl->md_pulse->hd->htp, p_chk, 0,
              0x100 | (2 << 1) | (2 << 4));
        chk_hist2->SetToolTipText("Double/Float");
        chk_hist2->Connect("Toggled(Bool_t)", "ParDlg", this,
                           "DoDaqChk(Bool_t)");
  ...
  fbut->SetToolTipText(
      "Add new type of 2d histogram. Low and Upper edge on\nX and Y axes are "
      "taken from the corresponding 1d histogram.");
...
void HistParDlg::AddLine_hist(TGCompositeFrame *frame, Mdef *md) {
  ...
  id = Plist.size() + 1;
  TGNumberEntry *fNum1 = new TGNumberEntry(
      hfr1, 0, 0, id, k_r0, TGNumberFormat::kNEAAnyNumber, lim, 0.0001, 10000);
  ...
  // xlow
  id = Plist.size() + 1;
  TGNumberEntry *fNum2 = new TGNumberEntry(
      hfr1, 0, 0, id, k_r0, TGNumberFormat::kNEAAnyNumber, nolim);
  ...
  // xup
  id = Plist.size() + 1;
  TGNumberEntry *fNum3 = new TGNumberEntry(
      hfr1, 0, 0, id, k_r0, TGNumberFormat::kNEAAnyNumber, nolim);
  ...
  // rebin
  id = Plist.size() + 1;
  TGNumberEntry *fNum4 = new TGNumberEntry(
      hfr1, 0, 0, id, k_r0, TGNumberFormat::kNEAPositive, lim, 1, 1000);
  DoMap(fNum4->GetNumberEntry(), &md->hd->rb, p_inum, 0,
        4 << 1 | 3 << 4); // cyan + Update
...
void HistParDlg::Add_prof_num(TGHorizontalFrame *hfr1, void *nnn, Int_t max,
  ...
  id = Plist.size() + 1;
  TGNumberEntryField *fNum1 = new TGNumberEntryField(
      hfr1, id, 0, k_int, TGNumberFormat::kNEAAnyNumber, lim, 1, max);
  DoMap(fNum1, nnn, pp, 0, cmd);
  fNum1->SetToolTipText(tip);
  fNum1->SetWidth(ww1);
  fNum1->Connect("TextChanged(char*)", "ParDlg", this, "DoDaqNum()");
...
void HistParDlg::AddLine_prof(TGHorizontalFrame *hfr1, Mdef *md) {
  ...
  Add_prof_num(hfr1, &opt.prof_nx, 16, p_inum, 0x100 | (2 << 4),
               "Number of X-strips from ING-27");
  Add_prof_num(hfr1, &opt.prof_ny, 16, p_inum, 0x100 | (2 << 4),
               "Number of Y-strips from ING-27");
  Add_prof_num(hfr1, &md->hd->bins, 64, p_fnum, 0x100 | (2 << 4),
               "Number of X-bins in profilometer histograms");
  Add_prof_num(hfr1, &md->hd->bins2, 64, p_fnum, 0x100 | (2 << 4),
               "Number of Y-bins in profilometer histograms");
  Add_prof_num(hfr1, &md->hd->rb, 64, p_inum, 4 << 1 | 3 << 4,
               "Rebin X (only for drawing)");
  Add_prof_num(hfr1, &md->hd->rb2, 64, p_inum, 4 << 1 | 3 << 4,
               "Rebin Y (only for drawing)");
...
#ifdef YUMO
void HistParDlg::Add_yumo(TGGroupFrame *frame, Mdef *md1, Mdef *md2,
                          Mdef *md3) {
  ...
  Num_opt(hfr1, 50, &md1->hd->min, 0, "Low edge", k_r0, 0, 0, 0x100 | (2 << 4),
          LayLT2);
  Num_opt(hfr1, 50, &md1->hd->max, 0, "Upper edge", k_r0, 0, 0,
          0x100 | (2 << 4), LayLT2);
  Num_opt(hfr1, 40, &md1->hd->rb, 0, "Rebin (only for drawing)", k_int, 1, 1000,
          4 << 1 | 3 << 4, LayLT2);
  ...
  Num_opt(hfr1, 50, &md2->hd->min, 0, "Low edge", k_r0, 0, 0, 0x100 | (2 << 4),
          LayLT2);
  Num_opt(hfr1, 50, &md2->hd->max, 0, "Upper edge", k_r0, 0, 0,
          0x100 | (2 << 4), LayLT2);
  ...
  hfr1 = new TGHorizontalFrame(frame);
  frame->AddFrame(hfr1, LayLT1b);
  Num_opt(hfr1, 40, &opt.yumo_a, 0, "Anode channel", k_int, 0, 31, 0x100,
          LayLT0);
  ...
  hfr1 = new TGHorizontalFrame(frame);
  frame->AddFrame(hfr1, LayLT1b);
  Num_opt(hfr1, 50, &opt.yumo_peak1, 0, "left border of the peak", k_r0, 0, 0,
          0x100, LayLT0);
  Num_opt(hfr1, 50, &opt.yumo_peak2, 0, "right border of the peak", k_r0, 0, 0,
          0x100, LayLT0);
...
void HistParDlg::AddLine_2d(TGGroupFrame *frame, Mdef *md) {
  ...
  // nbins2 (y-axis)
  id = Plist.size() + 1;
  fNum1 = new TGNumberEntry(hfr1, 0, 0, id, k_r0, TGNumberFormat::kNEAAnyNumber,
                            lim, min2, max2);
  DoMap(fNum1->GetNumberEntry(), &md->hd->bins2, p_fnum, 0,
        0x100 | (2 << 4) | col);
  ...
  fNum1 = new TGNumberEntry(hfr1, 0, 0, id, k_r0, TGNumberFormat::kNEAAnyNumber,
                            lim, 0, 1000);
  DoMap(fNum1->GetNumberEntry(), &md->hd->rb, p_inum, 0,
        4 << 1 | 3 << 4); // cyan + Update
  ...
  fNum1 = new TGNumberEntry(hfr1, 0, 0, id, k_r0, TGNumberFormat::kNEAAnyNumber,
                            lim, 0, 1000);
  DoMap(fNum1->GetNumberEntry(), &md->hd->rb2, p_inum, 0,
        4 << 1 | 3 << 4); // cyan + Update
...
void HistParDlg::Add2d() {
  // cout << "Add2d: " << cmb1->GetSelected() << " " << cmb2->GetSelected() <<
  // endl;
...
ChanParDlg::ChanParDlg(const TGWindow *p, UInt_t wdth, UInt_t h)
  ...
  // YKWheel
  // vscroll->Connect("PositionChanged(Int_t)", "ChanParDlg", this,
  // "DoScroll()");
...
void ChanParDlg::Build() {
  ...
  if (NFLD != nfld) {
    prnt("ss d ds;", BRED, "NFLD constant is not set properly:", NFLD, nfld,
         RST);
    EExit(-1);
  }

  // for (int i=0;i<nfld;i++) {
  //   cout << "Plist: " << i << " " << Plist[nfld+i].off << " " << (int)
  //   Plist[nfld+i].step << endl;
  // }
  ...
  TGCompositeFrame *vfr =
      (TGCompositeFrame *)fCnv[2]->GetViewPort()->GetContainer();
  ...
  AddLine_opt(cGrp, ww, (void *)"C1", (void *)"C2", tip1, tip2, "", k_lab,
              k_lab, 0, 0, 0, 0, 0, 0, LayLC1, LayLC2);

  tip1 = "Width of coincidence window for Group 1 (in samples)";
  tip2 = "Width of coincidence window for Group 2 (in samples)";
  label = "Windows";
  AddLine_opt(cGrp, -ww, cpar.coinc_w, cpar.coinc_w + 1, tip1, tip2, label,
              k_int, k_int, 1, 1023, 1, 1023, 1, 1, LayLC1, LayLC2);

  tip1 = "Minimal multiplicity for Group 1";
  tip2 = "Minimal multiplicity for Group 2";
  label = "Min mult ";
  AddLine_opt(cGrp, -ww, cpar.mult_w1, cpar.mult_w1 + 1, tip1, tip2, label,
              k_int, k_int, 0, 255, 0, 255, 1, 1, LayLC1, LayLC2);

  tip1 = "Maximal multiplicity for Group 1";
  tip2 = "Maximal multiplicity for Group 2";
  label = "Max mult";
  AddLine_opt(cGrp, -ww, cpar.mult_w2, cpar.mult_w2 + 1, tip1, tip2, label,
              k_int, k_int, 0, 255, 0, 255, 1, 1, LayLC1, LayLC2);
...
void ChanParDlg::BuildColumns(int jj) {
  ...
  AddColumn(jj, kk++, 1, p_chk, 24, 1, 0, 0, 0, "dsp", opt.dsp, opt.Dsp,
            1 | (8 << 4));
  ...
  AddColumn(jj, kk++, 1, p_inum, 40, 1, 1, 0, 0, "Len", cpar.Len, 0,
            1 | (6 << 4));
  AddColumn(jj, kk++, 1, p_inum, 21, 1, 1, 0, 0, "G", cpar.G, 0, 1 | (5 << 4));
  AddColumn(jj, kk++, 1, p_inum, 21, 1, 1, 0, 0, "Trg", cpar.Trg, 0, 1);
  AddColumn(jj, kk++, 1, p_inum, 36, 1, 1, 0, 0, "Drv", cpar.Drv, opt.sDrv,
            1 | (8 << 4));
  AddColumn(jj, kk++, 1, p_inum, 40, 1, 1, 0, 0, "Thr", cpar.Thr, opt.sThr,
            1 | (8 << 4));
  AddColumn(jj, kk++, 1, p_inum, 30, 1, 1, 0, 0, "LT", cpar.LT, opt.sLT,
            1 | (8 << 4));
  ...
  for (int i = 1; i <= 4; i++) {
    sprintf(txt, "g%d", i);
    AddColumn(jj, kk++, 1, p_chk, 24, 0, 0, 0, 0, txt, opt.Grp, 0, 0x100,
              40 + i);
...
void ChanParDlg::AddColumn(int jj, int kk, int ii, P_Def pdef, int wd,
                           int dfile, int dev, double min, double max,
                           const char *pname, void *apar, void *apar2,
                           UInt_t cmd, int s2) {
  ...
  case p_fnum:
  case p_fnum2:
    AddNumPar(jj, kk, wd, all, dfile, dev, pdef, min, amax, hparl[ii][jj],
              pname, apar, off, cmd, apar2);
    break;
  case p_stat: {
...
void ChanParDlg::AddCombo(int j, int wd, int all, int daq,
...
void ChanParDlg::AddChkPar(int kk, int wd, int all, int dfile,
                           TGHorizontalFrame *hfr, void *apar, UShort_t off,
                           UInt_t cmd, void *apar2, UChar_t step) {
...
void ChanParDlg::AddNumPar(int j, int kk, int wd, int all, int dfile, int dev,
                           P_Def pdef, double min, double max,
                           TGHorizontalFrame *hfr, const char *name, void *apar,
                           UShort_t off, UInt_t cmd, void *apar2) {
  ...
  TGNumberEntryField *fNum = new TGNumberEntryField(
      hfr, id, 0, style, TGNumberFormat::kNEAAnyNumber, limits, min, max);
...
void ChanParDlg::AddStatDaq(int jj, int kk, int wd, void *apar,
                            TGHorizontalFrame *hfr) {
  // int col;

  int id = Plist.size() + 1;
  TGNumberEntryField *fNum = new TGNumberEntryField(
      hfr, id, 0, TGNumberFormat::kNESRealOne, TGNumberFormat::kNEAAnyNumber,
...
void ChanParDlg::HandleMouseWheel(Event_t *event) {
  ...
  // prnt("ss d d d d d d
  // ds;",BGRN,"Scroll:",event->fType,event->fCode,event->fState,hscroll->GetPosition(),hscroll->GetRange(),hscroll->GetPageSize(),
  // hscroll->GetRange()-hscroll->GetPageSize(),RST);
  ...
  if (opt.ScrollPos != oldscroll) {
    ...
    //   prnt("ss d d d d
    //   ds;",BRED,"Scroll:",event->fType,opt.ScrollPos,scrl,vscroll->GetRange(),vscroll->GetPageSize(),RST);
    //   scrl=0;
    // }
    // else {
    //   prnt("ss d d d d
    //   ds;",BGRN,"Scroll:",event->fType,opt.ScrollPos,scrl,vscroll->GetRange(),vscroll->GetPageSize(),RST);
    // }

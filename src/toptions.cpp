...
void Coptions::InitMinMax() {
  ...
  mhS = {0, 0, 0, 0, 0, 0, 0, 0, 512, 0, 512, 512, 512, 128, 128, 128};
  // мертвое время дискриминатора
  mDt = {0, 0, 1, 1, 1, 1, 1, 1, 16383, 0};
  fill_n(&mDt[MM + 2], 6, 16383);
  // предзапись M
  mPre = {-1023, 0, -1023, -511, -1023, -511, -511, -511, 8184, 8184};
  fill_n(&mPre[MM + 2], 6, 1024);
  // общая длина записи
  mLen = {1,     0,     1,    1,    1,    1,    1,     1,
          16379, 16379, 4068, 3048, 6114, 1506, 12000, 12000};
  // параметр производной
  mDrv = {1, 0, 1, 1, 1, 1, 1, 1, 1023, 1023, 1023, 1023, 1023, 255, 255, 255};
  // порог срабатывания
  mThr = {-65536, 0, -2048};
  ...
  mG = {0, 5, 5, 0, 0, 0, 0, 0, 12, 12, 12, 3, 3, 3, 4, 4};
  // задержка
  mhD = {0, 0, 0, 0, 0, 0, 0, 0, 4100, 0, 4075, 4092, 1023, 255, 250, 250};
  // тип срабатывания дискриминатора
  mTrg = {0, 0, 0, 0, 0, 0, 0, 0, 7, 1, 6, 6, 7, 6, 7, 7};
  // величина пересчета P ("незаписанных" срабатываний дискриминатора)
  mRD = {0, 0, 0, 0, 0, 0, 0, 0, 1023, 0, 0, 0, 1023, 1023, 1023, 1023};
...
void Coptions::GetParm(const char *name, int i, void *par, int &min, int &max) {
  ...
  try {
    ...
    prnt("ss s d d ss;", BRED, "ErrGetParm:", name, i, crs->crs_ch[i],
         oor.what(), RST);
    return;
  }
...
Int_t Coptions::ChkLen(Int_t i, Int_t module) {
  ...
  if (res < dd)
    res = dd;
  // prnt("ss d d d d ds;",BRED,"ChkLen:
  // ",i,Len[i],crs->crs_ch[i],module,res,RST);
  return res;
}
...
Toptions::Toptions() {
  ...
  */
  ...
  const char *types[MAX_TP] = {"NaI",   "BGO",   "HPGe",  "Si",
                               "Stilb", "Demon", "Plast", "NIM"};

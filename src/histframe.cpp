...
const short hcols[nhcols] = {1,  2,  3,  4,  5,  6,  7,  22,
                             33, 41, 46, 38, 30, 24, 29, 40};
...
TGLayoutHints *fLay9 =
    new TGLayoutHints(kLHintsCenterX | kLHintsBottom, 0, 0, 0, 0);
...
HistFrame::HistFrame(const TGWindow *p, UInt_t w, UInt_t h, Int_t nt)
  ...
  // TGLayoutHints* LayCB0   = new
  // TGLayoutHints(kLHintsCenterX|kLHintsBottom,0,0,0,0);
  TGLayoutHints *LayET0 =
      new TGLayoutHints(kLHintsExpandX | kLHintsTop, 0, 0, 0, 0);
  // TGLayoutHints* LayLT0   = new TGLayoutHints(kLHintsLeft|kLHintsTop,
  // 0,0,0,0);
  TGLayoutHints *LayLE0 = new TGLayoutHints(kLHintsLeft | kLHintsExpandY);
  TGLayoutHints *LayLE1 =
      new TGLayoutHints(kLHintsLeft | kLHintsExpandY, 3, 0, 0, 0);
  TGLayoutHints *LayLC1 =
      new TGLayoutHints(kLHintsLeft | kLHintsCenterY, 1, 1, 0, 0);
  TGLayoutHints *LayLC2 =
      new TGLayoutHints(kLHintsLeft | kLHintsCenterY, 2, 0, 0, 0);
  TGLayoutHints *LayLC3 =
      new TGLayoutHints(kLHintsLeft | kLHintsCenterY, 0, 5, 0, 0);
  // TGLayoutHints* LayLC4  = new
  // TGLayoutHints(kLHintsLeft|kLHintsCenterY,1,1,7,7);
  ...
  fHslider->Connect("PositionChanged()", "HistFrame", this, "DoSlider()");
  ...
  fVslider->Connect("PositionChanged()", "HistFrame", this, "DoSlider()");
  ...
  fListTree->Connect("Checked(TObject*, Bool_t)", "HistFrame", this,
                     "CheckToOpt(TObject*, Bool_t)");
  ...
  but->SetToolTipText(
      "Reset all histograms (works during acquisition/analysis)");
  ...
  but->SetToolTipText(
      "Delete all cuts. Use right mouse button on a single cut to delete it");
  fHor3->AddFrame(but, LayLC1);
  ...
  const char *ttip =
      "Formula for the cut.\nUse standard C and root operators and "
      "functions\nFormula turns red in case of an error\nUse [1] [2] [3] ... "
      "for other cuts in the formula\nExamples:\n [1] && [2] - cut1 \"and\" "
      "cut2\n [2] || [3] - cut2 \"or\" cut3\n ![3] - not cut3";
  ...
  // but->Connect("Clicked()", "HistFrame", this, "StartMouse()");
  but->SetToolTipText("Delete all ROI (not implemented). Use right mouse "
                      "button on a single ROI to delete it");
  fHor5->AddFrame(but, LayLC1);
  ...
  but->Connect("Clicked()", "HistFrame", this, "ClearCutG()");
  but->SetToolTipText("Delete all ROI. Use right mouse button on a ROI to delete
  it"); fHor5->AddFrame(but, LayLC1);
  ...
  */
  ...
  for (int i=0;i<2;i++) {
    fNum[i] = new TGNumberEntry(fHor2, div[i], 2, i+10,
                                ...
                                1,16);
    fNum[i]->GetNumberEntry()->Connect("TextChanged(char*)", "HistFrame", this,
  "DoNum()"); fNum[i]->GetNumberEntry()->SetToolTipText(ttip2[i]);
  }
  */

  for (int i = 0; i < 2; i++) {
    int id = parpar->Plist.size() + 1;
    fNum[i] = new TGNumberEntry(fHor2, 0, 2, id, TGNumberFormat::kNESInteger,
                                TGNumberFormat::kNEAAnyNumber,
                                TGNumberFormat::kNELLimitMinMax, 1, 16);

    fNum[i]->GetNumberEntry()->SetToolTipText(ttip2[i]);
    parpar->DoMap(fNum[i]->GetNumberEntry(), div[i], p_inum, 0, 3 << 4);
    fNum[i]->GetNumberEntry()->Connect("TextChanged(char*)", "ParDlg", parpar,
                                       "DoDaqNum()");
  }
  ...
  TGButtonGroup *cuts = new TGButtonGroup(fHor2, "Horizontal
  Position",kChildFrame|kHorizontalFrame);
  ...
  "DoHPosition(Int_t)"); fHor2->AddFrame(cuts, LayLC1);
  */
  ...
  */
  ...
  fHor2->AddFrame(chkd, LayLC1);
  chkd->SetToolTipText(
      "Find peaks in all visible windows (works only in X/Y mode");

  // but = new TGTextButton(fHor2,"Peaks",4);
  // but->Connect("Clicked()", "HistFrame", this, "DoPeaks()");
  // fHor2->AddFrame(but, LayLC1);
  // but->SetToolTipText("Find peaks in all visible windows (works only in X/Y
  // mode");
...
TGListTreeItem *HistFrame::Item_Ltree(TGListTreeItem *parent,
...
void HistFrame::DoClick(TGListTreeItem *item, Int_t but) {
  ...
  if (but == 2 &&
      TString(item->GetText()).EqualTo("MAIN", TString::kIgnoreCase)) {
    ClearMAIN();
  }

  if (item->GetParent() && (but == 2 || but == 3)) {
    if (TString(item->GetParent()->GetText())
            .Contains("MAIN", TString::kIgnoreCase)) {
...
void HistFrame::OptToCheck() {
  ...
  while (idir) {
    TGListTreeItem *item = idir->GetFirstChild();
    bool b_main =
        TString(idir->GetText()).Contains("MAIN", TString::kIgnoreCase);
    int ncut = 0;
    if (b_main) {
      ncut = stoi(numstr(idir->GetText()));
    }
    // cout << "ct2: " << idir->GetText() << " " << b_main << " " << ncut <<
    // endl;
    ...
    while (item) {
      if (b_main) { // папка MAIN*
        ...
        // prnt("ss s s d d
        // ds;",BBLU,"main2:",idir->GetText(),item->GetText(),ncut,i,(int)getbit(opt.wrk_check[i],ncut),RST);
        i++;
        if (i >= M_WCHK)
          break;
      } else { // не MAIN*
  ...
  while (idir) {
    ...
    while (item) {
      if (b_main) { // папка MAIN*
        ...
        // prnt("ss s s d d
        // ds;",BGRN,"main1:",idir->GetText(),item->GetText(),ncut,i,(int)getbit(opt.wrk_check[i],ncut),RST);
        i++;
        if (i >= M_WCHK)
          break;
      } else { // не MAIN*
...
void HistFrame::GetHMinMax(TH1 *hh, double x1, double x2, double &y1,
                           double &y2) {
  for (int i = hh->FindFixBin(x1); i <= hh->FindFixBin(x2); i++) {
    ...
    if (opt.b_logy) {
      if (cc > 0)
        y1 = TMath::Min(y1, cc);
      if (y1 == 1)
        y1 = 0.5; // для логарифма минимум - половина от единицы
    } else {
      y1 = TMath::Min(y1, cc);
    }
    y2 = TMath::Max(y2, cc);
...
void HistFrame::Y_Slider(TH1 *hh, double a1, double a2, double y1, double y2) {
  ...
  // prnt("ss f f f f f f f
  // fs;",BGRN,"Sl:",opt.hy_slider[0],opt.hy_slider[1],h1,h2,y1,y2,hh->GetYaxis()->GetXmin(),hh->GetYaxis()->GetXmax(),RST);
  ...
  // prnt("ss s f f f
  // fs;",BBLU,"Y1:",hh->GetName(),y1,y2,log10(y1),log10(y2),RST);
...
void HistFrame::ItemROI(HMap *map, int iroi) {
  ...
  TGListTreeItem *item =
      fRoiTree->AddItem(0, name, map->hd->roi[iroi], pic_c1, pic_c1, false);
  for (int j = 0; j < 2; j++) {
...
void HistFrame::CutClick(TGListTreeItem *item, Int_t but) {
  ...
  // cout << "CutClick: " << item->GetText() << " " << item->GetUserData() << "
  // " << item->GetParent() << " " << but << endl;
  ...
  if (gcut) {
    ...
    // ss.ReplaceAll("]","");
    int icut = ss.Atoi();
    // cout << "cutclick2: " << gcut->GetName() << " " << gcut->GetTitle() << "
    // " << ss << " " << icut << endl;

    if (icut < 0 && icut >= MAXCUTS) {
      cout << "wrong cut number: " << icut << ". Consider clearing all cuts"
           << endl;
      return;
    }
    // icut--;
...
void HistFrame::StartMouse() {
  ...
  if (id == 1 && !opt.b_stack) { // add Cut
    ...
    if (icut < 0) {
      ...
      Int_t retval;
      new TGMsgBox(gClient->GetRoot(), this, "Graphical cut", msg,
...
string HistFrame::CutsToStr() {
  ...
  for (int i = 1; i < opt.ncuts; i++) {
    if (opt.pcuts[i]) {
      ...
      TCutG *gcut = hcl->cutG[i];
      if (gcut->GetN() > 1) {
        oss << gcut->GetName() << " " << gcut->GetN() << " " << gcut->GetTitle()
            << endl;
        ;
        for (int j = 0; j < gcut->GetN(); j++) {
...
void HistFrame::EditCutG() { new PEditor(this, M_EDIT_CUTG, 400, 520); }

void hsmooth(TH1 *h1, int nn) {
  // valid only for 1-D hists

  if (h1->GetDimension() != 1 || nn <= 0)
    return;
...
void hderiv(TH1 *hh) {
  ...
  for (auto i = hh->GetXaxis()->GetFirst() + 1; i < hh->GetXaxis()->GetLast();
       i++) {
...
void HistFrame::PeakSearch(TH1 *h1, vpeaks &vv) {
  ...
  // std::vector<vpeak> vv;
  int inpeak =
      0; // 0 - не в пике; 1 - в пике, сигма не найдена; 2 - сигма найдена
  ...
  vpeak pp = {0, -1e9, 1e9};
  // cout << hh->GetXaxis()->GetFirst() << " " << hh->GetXaxis()->GetLast() << "
  // " << bkg << " " << hmax << endl;
  for (auto i = n1; i <= n2; i++) {
  ...
  for (auto it = vv.begin(); it != vv.end(); ++it) {
    ...
    // prnt("ss d f f f
    // fs;",BGRN,"peak:",distance(vv.begin(),it),it->p,it->h,it->w,bkg,RST);
  }
...
void HistFrame::DoPeaks(TH1 *hh, int npad) {
  // cout << "DoPeaks: " << opt.b_stack << " " <<
  // fEc->GetCanvas()->GetListOfPrimitives()->GetSize() << " " <<
  // pad_hist.size() << endl;
  ...
  for (size_t i = 0; i < vv->size(); i++) { // цикл по найденным пикам
    ...
    hh->GetListOfFunctions()->Add(
        new TLine(vv->at(i).b1, y1, vv->at(i).b1, y2));
    hh->GetListOfFunctions()->Add(
        new TLine(vv->at(i).b2, y1, vv->at(i).b2, y2));
  ...
  // cout << "fits: " << fits->hst->GetName() << " " << fits->hst->GetTitle() <<
  // endl;

} // DoPeaks

void HistFrame::E_PeakFit(HMap *map, TH1 *hist1, TH1 *hist2, int nn,
                          d2vect &d2) {
  // double &yy, double &er) {
  ...
  for (UInt_t i = 2; i < ee_calib[nn].size(); i += 2) {
    ...
    if (ff > 0 && ff < 1) { // fixed peak
      ...
      cout << "srch: " << map->hst->GetName() << " " << iroi << " " << b1 << " "
           << b2 << " " << imax << " " << max << " " << sig << endl;
    } // else (search)
...
void HistFrame::Do_Ecalibr(PopFrame *pop) {
  ...
  while ((obj = (TObject *)next())) {
    ...
    if (map->hd == &opt.h_area) {
      ...
      TGraphErrors *gr = new TGraphErrors(d2[0].size(), d2[0].data(),
                                          d2[1].data(), d2[2].data(), 0);
...
void HistFrame::HiUpdate() {
  // cout << "in_gcut: " << in_gcut << " " << opt.b_logy << " " << chklog <<
  // endl;
  ...
  // cout << "hist_list: " << endl;
  // hcl->hist_list->ls();
  // cout << "hist_list: " << hcl->hist_list << " " << hcl->hist_list->GetSize()
  // << endl;
...
void HistFrame::DrawStack() {
  ...
  if (hmap_chklist->GetSize() > 16) {
    ...
    char ss[200];
    sprintf(ss, "Number of histograms is too large for Stack: %d (max 16)",
            hmap_chklist->GetSize());
    ...
    while ((obj = (TObject *)next())) {
      if (ii >= opt.icheck) {
        ...
        if (npad + 1 != (int)pad_map.size()) {
          cout << "pad_map size doesn't match: " << npad << " "
               << pad_map.size() << endl;
          return;
        }
        // pad_map[npad]=map;
...
void HistFrame::OneRebinPreCalibr(HMap *&map, TH1 *&hist, bool badj) {
  // Float_t *hold, *hnew;
  // cout << "Class: " << hist->Class() << " " << hist->ClassName() << " " <<
  // map->hd->htp << endl;

...
PopFrame::PopFrame(const TGWindow *main, UInt_t w, UInt_t h, MENU_COM menu_id,
  ...
  LayCC4 =
      new TGLayoutHints(kLHintsCenterX | kLHintsCenterY, 150, 150, 20, 150);
...
void PopFrame::AddProfTime(UInt_t w, UInt_t h) {
  ...
  fMain->AddFrame(
      fCanvas, new TGLayoutHints(kLHintsExpandX | kLHintsExpandY, 1, 1, 1, 1));
  ...
  // fMain->AddFrame(fOK, new TGLayoutHints(kLHintsCenterX|kLHintsBottom, 0, 0,
  // 5, 5));
}
...
void PopFrame::AddAdj(TGCompositeFrame *fcont1, HMap *map, int i) {
  ...
  for (int j = 0; j < 3; j++) {
    ...
    fAdj[i][j] =
        new TGNumberEntryField(hframe, i * 10 + j, opt.adj[i][j], kr, ka, kn);
...
void PopFrame::AddEcalibr(UInt_t w, UInt_t h) {
  ...
  const char *txt =
      ...
      "like area_07 and area_07_cut1, only the first one will be "
      "calibrated.\n\n"
      "For 1-point pre-calibration:\n"
      "- edit reference value for the pre-calibration;\n"
      "- use scrollbar in the Plots tab to select window in such a way that "
      "the peak\n"
      ...
      "- press Save to copy the calibration coefficients to channels' "
      "parameters;\n"
  ...
  fAuto->SetToolTipText(
      "Auto pre-calibrate using the highest peak in the range");
  ...
  TGCompositeFrame *fcont1 =
      new TGCompositeFrame(fCanvas1->GetViewPort(), 10, 10, kVerticalFrame);
  ...
  fMain->AddFrame(
      hframe, new TGLayoutHints(kLHintsExpandX | kLHintsBottom, 2, 2, 2, 2));
  ...
  fEApply->SetToolTipText("Apply calibration parameters for next analysis "
                          "(copy A0-A2 coefficients to E0-E2)");
  ...
  fRevert->SetToolTipText(
      "Revert to the initial pre-calibration coefficients A0-A2");
...
void PopFrame::AddTcalibr() {
  ...
  const char *txt =
      ...
      " - enter a reference value to which all peak positions will be "
      "adjusted;\n"
  ...
  hframe = new TGHorizontalFrame(fMain, 10, 10);
  // fMain->AddFrame(hframe,new TGLayoutHints(kLHintsExpandX|kLHintsCenterY,
  // 2,2,2,2));
...
void PopFrame::AddPeaks() {
  ...
  AddNum(opt.Peak_thr, 1031, "Threshold",
         "Peak threshold relative to the highest peak");
  AddNum(opt.Peak_smooth, 1032, "Smooth", "Smooth before peak search");
  AddNum(opt.Peak_wid, 1033, "Width",
         "Minimal width for peak search (in units of the X-axis)\npeaks with "
         "width below this number are rejected");
  AddNum(opt.Peak_bwidth, 1034, "Background",
         "width of the background (in units of the X-axis)");
  ...
  hframe = new TGHorizontalFrame(fMain,10,10);
  fMain->AddFrame(hframe,new TGLayoutHints(kLHintsExpandX|kLHintsCenterY,
  2,2,2,2)); TGTextButton* fCalibr = new TGTextButton(hframe, "  &Calibr  ");
...
void PopFrame::AddOnePar(TGCompositeFrame *fcont1, int id, void *var,
  ...
  fNum[0] = new TGNumberEntry(hframe, opt.OptPar[id][1] - 1, 8, id * 10 + 1, kr,
                              ka, kl, mmin, mmax);
  fNum[1] = new TGNumberEntry(hframe, opt.OptPar[id][2] - 1, 8, id * 10 + 2, kr,
                              ka, kl, mmin, mmax);
  fNum[2] = new TGNumberEntry(hframe, opt.OptPar[id][3] - 1, 5, id * 10 + 3, kr,
                              ka, kl, -10, 10);

  for (int j = 0; j < 3; j++) {
    fNum[j]->GetNumberEntry()->Connect("TextChanged(char*)", "PopFrame", this,
                                       "DoPar()");
...
void PopFrame::AddOptPar() {
  ...
  hframe = new TGHorizontalFrame(fMain, 10, 10);
  fMain->AddFrame(
      hframe, new TGLayoutHints(kLHintsExpandX | kLHintsCenterY, 2, 2, 2, 2));
...
void PopFrame::AddTest() {
  ...
  fTest->SetToolTipText(
      "Stop-Pusk (cmd4-cmd3) N times.\nAcquisition must be started before.");
...
void PopFrame::Do_EApply() {
  for (int i = 0; i < MAX_CH; i++) {
    if (fAdj[i][0]) {
      ...
      case 2:
        prnt("ss d ss;", BRED, "quadratic calibration for ch:", i,
             "not possible", RST);
        break;
      case 1:
        // prnt("ss d f f fs;",BGRN,"E1:",i,opt.E0[i],opt.E1[i],opt.E2[i],RST);
        opt.E2[i] = opt.adj[i][2] * opt.E1[i] * opt.E1[i];
        opt.E1[i] = opt.E1[i] * (opt.adj[i][1] + 2 * opt.adj[i][2] * opt.E0[i]);
        opt.E0[i] = opt.adj[i][0] +
                    opt.E0[i] * (opt.adj[i][1] + opt.adj[i][2] * opt.E0[i]);
...
void PopFrame::Do_OptPar() {
  ...
  for (auto it = pss.begin(); it != pss.end(); ++it) {
    ...
    for (UInt_t k = 0; k < it->pv.size(); ++k) {
      ...
      // cout << k << " " << it->pv.at(k).var << " " << &opt.sS << " " <<
      // opt.sS[7] << " " << opt.sTg[4] << endl;
    }
    ...
    for (UInt_t i = 0; i < HiFrm->fitvect.size(); ++i) {
      if (HiFrm->pad_hist[i] &&
          HiFrm->pad_hist[i]->InheritsFrom(TH1::Class())) {
        FClass *fits = &HiFrm->fitvect.at(i);

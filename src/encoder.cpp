...
void Encoder::Encode_Start(int rst, int mm, bool &bb, int cc, UChar_t *BBuf,
  ...
  if (opt.nthreads > 1) {
    ...
    // if (opt.raw_write) {
    //   trd_raw_write = new TThread("trd_raw_write", handle_raw_write, (void*)
    //   0); trd_raw_write->Run();
    // }
...
int Encoder::Write3(UChar_t *buf, int len) {
  ...
  if (len) { // пишем непустой буфер
    ...
    if (res != len) {
      msg = TString("Error writing to file: ") + wr_name.c_str() + " " + res +
            " " + len;
      EError(1, 1, 1, msg);
      wr_bytes += res;
      *b_wrt = false;
  ...
  if (opt.wdog_timer > 0 && dt > 60) { // 60 seconds
    ...
    if (dt1 > opt.wdog_timer) {
      ...
      if (dd < opt.wdog1 || dd > opt.wdog2) { // alert!
        ...
        char msg[200];
        sprintf(msg,
                "File %s growth: %0.1f%% of average:"
                " %0.1f %0.1f",
                wr_name.c_str(), dd, rate, wrate_mean);
...
void Encoder::Handle_write() {
  ...
  while (wrt_thread_run.load() || !buf_list.empty()) {
    ...
    cv_buf.wait(
        lock, [this]() { return !buf_list.empty() || !wrt_thread_run.load(); });
...
//------------------------
ERaw::ERaw() {
  // cout << "перенести Fill_Raw в ERaw (search Fill_Raw in libcrs.cpp)" <<
  // endl;
  buf_size = 2 * Megabyte;
}

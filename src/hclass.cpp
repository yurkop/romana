...
Float_t Mdef::VarTime(EventClass *e, PulseClass *p) {
  ...
  if (p->Chan == e->ChT0 && p->Time == e->T0 &&
      opt.hideself) // канал, в котором T0
    return -99999;
  else
...
Float_t Mdef::VarPeriod(EventClass *e, PulseClass *p) {
  ...
  // prnt("ss d l l
  // fs;",BRED,"varperiod:",p->Chan,e->Tstmp,LPrev[p->Chan],tt[p->Chan],RST);
...
void Mdef::Time_Extend(UChar_t ch, Double_t T) {
  ...
  if (T > max) {
    if (T - max > 1e4) { // larger than several hours
      cout << "Time leap is too large: " << crs->nevents << " " << T << " "
           << max << " " << crs->Tstart64 << endl;
    } else {
      Double_t rt = T / max;
      int nn = 2;
      while (rt > 2) {
    ...
    if (cpar.on[ipls->Chan] && ipls->Pos > -32222) {
      ...
      for (int j = 0; j < NGRP; j++) {
        if (opt.Grp[ipls->Chan][j])
          mult[j]++;
        // prnt("sd d d
        // fs;",BGRN,ipls->Chan,j,opt.Grp[ipls->Chan][j],mult[j],RST);
      }
    }
  }
...
void Mdef::Fill_Mean1(HMap *map, Float_t *Data, int nbins, int ideriv,
                      int ncut) {
  if (!map)
    return;
  if (ncut) {
...
void Mdef::Fill_FFT(HMap *map, Float_t *Data, int nbins, int ch, int ncut) {
  ...
  for (int j = 0; j < nbins; j++) {
    ...
    val =
        map->hst->GetBinContent(j + 1) * nent + TMath::Sqrt(re * re + im * im);
    map->hst->SetBinContent(j + 1, val / (nent + 1));
  }
    ...
    for (auto ipls = evt->pulses.begin(); ipls != evt->pulses.end(); ++ipls) {
      ...
      if (pp >= crs->PROF_64) {
        ...
        if (h_xy >= 0) { // one of Prof64 position channels
          ...
          int dt =
              (ipls->Tstamp64 - evt->Tstmp) - cpar.Pre[ipls->Chan] - evt->T0;
      ...
      if (h_sum[0][i] > opt.Prof64_THR) {
        ...
        for (int j = 0; j < 64; j++) {
          if (h_sum[1][j] > opt.Prof64_THR) {
            Fill_02(hcl->md_prof_int->v_map[4], i + 0.5, j + 0.5, hcut_flag,
                    ncut);
            Fill_02(hcl->md_prof_int->v_map[5],
                    opt.Prof64_X + (i + 0.5) * 1.875 - 60,
                    opt.Prof64_Y + (j + 0.5) * 1.875 - 60, hcut_flag, ncut);
          } // if Y
        }
...
bool Mdef::is2d() { return hnum > 100 && hnum < 10000; }
//------------------------------
...
void HClass::Make_hist() {
  ...
  for (auto it = Mlist.begin(); it != Mlist.end(); ++it) {
    ...
    if (it->hnum > 0 && it->hnum < 10) { // standard pulse variable
      ...
      prnt("ss s ds;", BRED, "Wrong histogram type:", it->h_name.Data(),
           it->hnum, RST);
      EExit(-1);
    }
...
void NameTitle(char *name, char *title, int i, int maxi, const char *nm,
               const char *axis) {
  if (maxi == 0) {
    sprintf(name, "%s", nm);
...
void HClass::HHist2(mdef_iter md, TH1 *&hh, char *name, char *title, int i,
                    int n1, float min1, float max1, int n2, float min2,
                    float max2) {
  if (md->hd->htp)
...
void HClass::HHist3(mdef_iter md, TH1 *&hh, char *name, char *title, int i,
                    int n1, float min1, float max1, int n2, float min2,
                    float max2, int n3, float min3, float max3) {
  if (md->hd->htp)
...
void HClass::Make_prof(mdef_iter md) {
  ...
  for (int j = opt.prof_ny - 1; j >= 0; j--) {
    ...
    for (int k = 0; k < opt.prof_nx; k++) {
      ...
      TH2F *hh = new TH2F(name2, title2, md->hd->bins, opt.Prof64_X - 60,
                          opt.Prof64_X + 60, md->hd->bins2, opt.Prof64_Y - 60,
                          opt.Prof64_Y + 60);
      MapHist(md, hh, i);
    }
  }
...
void HClass::Make_prof_int(mdef_iter md, Hdef *hd2) {
  ...
  const char *name3[] = {"prof_x",  "prof_y",  "prof_ax",
                         "prof_ay", "prof_nm", "prof_xy"};
...
int HClass::Make_2d(mdef_iter md) {
  ...
  if (m1 == Mlist.end() || m2 == Mlist.end()) {
    prnt("ss s d ds;", BRED, "Can't make 2d histograms:", md->name.Data(), id1,
         id2, RST);
    return 0;
  }
  ...
  // md->tip = "2-dimensional histogram: "+md->name+"\nMin Max are taken from
  // the corresponding 1d histograms";
  ...
  if (id1 != id2) { // normal 2d
    ...
    for (int i = 0; i < opt.Nchan; i++) {
      if (cpar.on[i]) {
        ...
        if (md->hd->htp)
          hh = new TH2D(name2, title2, n1, m1->hd->min, m1->hd->max, n2,
                        m2->hd->min, m2->hd->max);
        else
          hh = new TH2F(name2, title2, n1, m1->hd->min, m1->hd->max, n2,
                        m2->hd->min, m2->hd->max);
    ...
    for (int y = 1; y < nmax; y++) {
      ...
      for (int x = 0; x < y; x++) {
        if (cpar.on[x] && cpar.on[y]) {
          ...
          if (md->hd->htp)
            hh = new TH2D(name2, title2, n1, m1->hd->min, m1->hd->max, n1,
                          m1->hd->min, m1->hd->max);
          else
            hh = new TH2F(name2, title2, n1, m1->hd->min, m1->hd->max, n1,
                          m1->hd->min, m1->hd->max);
          MapHist(md, hh, ii);
        } // if cpar.on
      } // for x
...
void Mdef::FillYumo(EventClass *evt, Double_t *hcut_flag, int ncut) {
  ...
  if (t01 && t23) {
    ...
    if (sx >= opt.yumo_peak1 && sx <= opt.yumo_peak2 && sy >= opt.yumo_peak1 &&
        sy <= opt.yumo_peak2) {
      if (hcl->md_yumo1) {
      ...
      if (hcl->md_yumo3) {
        Fill_03(hcl->md_yumo3->v_map[0], tx, ty, ntof, hcut_flag,
                ncut); // Yumo_3d
      }
    }
  }
...
void HClass::Make_Yumo_2d(mdef_iter md) {
  ...
  // 2d
  HHist2(md, hh, name, title, 0, nn, md->hd->min, md->hd->max, nn, md->hd->min,
         md->hd->max);

  strcat(name, "_w");
  strcpy(title, name);
  strcat(title, ";X(ns);Y(ns)");
  HHist2(md, hh, name, title, 1, nn, md->hd->min, md->hd->max, nn, md->hd->min,
         md->hd->max);
...
void HClass::Clone_Hist(HMap *map) {
  ...
  //   hcut->Reset();
  //   hcut->SetNameTitle(name,htitle);
  //   //cout << "clone: " << i << " " << hcut->GetName() << " " << gStyle <<
  //   endl; hist_list->Add(hcut); HMap* mcut = new
  //   HMap(cutname,hcut,map->chk,&wfalse,map->cut_index);

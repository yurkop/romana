...
const char *mgr_title[NDIV] = {"pulse;samples", "1 deriv;samples",
                               "2 deriv;samples", "CFD;samples", "FFT;FFT"};

const char *drv_name[NDIV] = {"Pulse", " ' ", " ' ' ", "CFD", "FFT"};
const char *drv_tip[NDIV] = {
    "Show pulse", "Show 1st derivative", "Show 2nd derivative", "Show CFD",
    "Show FFT",
};
...
const char *fPeakName[MXPK] = {
    "Peaks", //[0]
    "Pos",    "Time",    "WBase",  "Wpeak",
    "WTime", //[5]
    "WWidth", "*TStart", "Thresh", "Val",
    "Stat", //[10]
    "Norm",   "Prof",
    "Shape", //[13]
};
...
//: TGCompositeFrame(p,w,h,kVerticalFrame)
{
  ...
  fHslider->Connect("PositionChanged()", "EventFrame", this, "DoSlider()");
  ...
  fVslider->Connect("PositionChanged()", "EventFrame", this, "DoSlider()");
  ...
  TGNumberEntry *fNum2 =
      new TGNumberEntry(fHor_but, 0, 0, id, TGNumberFormat::kNESInteger,
  ...
  const char *ttip = "Checkpoint: search backward for an event which satisfies "
  ...
  ttip = "Checkpoint: search forward for an event which satisfies the "
         "following condition";
  ...
  ttip = "Print events from First to Last.\nThe data are saved to file "
         "'events.dat' in the current directory.";
  ...
  ttip = "Print peaks from First to Last.\nThe data are saved to file "
         "'peaks.dat' in the current directory.";
  ...
  const TGFont *font =
      pool->GetFont("misc", -10, kFontWeightNormal, kFontSlantRoman);
  // font->Print();
...
bool EventFrame::fChn_on(int ch) { return ch < opt.Nchan && fChn[ch]->IsOn(); }
...
void EventFrame::AddCh() {
  ...
  // fGroupCh->SetLayoutManager(new TGTableLayout(fGroupCh, 16, 0, kFALSE, 2,
  // kLHintsLeft));
...
void EventFrame::Rebuild() {
  // cout << "Event::Rebuild: " << opt.Nchan << " " << fDock->GetUndocked() <<
  // endl;
...
void EventFrame::ShPtr(int zz) {
  ...
  while ((ptr = (TGFrameElement *)next())) {
    cout << zz << "Layout:" << nn << " " << ptr->fState << " " << kIsVisible
         << endl;
    nn++;
  }
}
  ...
  // formula is valid
    ...
    while (d_event != ievt) {
      ...
      for (pulse_vect::iterator ipls = d_event->pulses.begin();
        ...
        if (res) {
          // prnt("ss f f f
          // fs;",BYEL,"res:",res,par[6],ipls->Time,d_event->T0,RST); YK cout <<
          // "tt2: " <<
          DrawEvent2();
          return;
        }
...
void EventFrame::DoFormula() {
  ...
  // cout << "DoFormula: " << strlen(te->GetText()) << " " << te->GetText() << "
  // " << opt.formula << endl;
}
...
void EventFrame::FillDeriv1(int dr, int i, PulseClass *pulse, double dt) {
  ...
  if (kk < 1)
    kk = 1;
  if (kk > (Int_t)pulse->sData.size())
    kk = (Int_t)pulse->sData.size();
...
void EventFrame::FillGraph(int dr) {
  ...
  for (UInt_t i = 0; i < d_event->pulses.size(); i++) {
    ...
    case 2: { // 2nd derivaive
      ...
      if (kk < 1 || kk >= (Int_t)pulse->sData.size())
        kk = 1;
      ...
      for (Int_t j = 0; j < (Int_t)pulse->sData.size(); j++) {
        ...
        if (j < kk + 1)
          ...
          dd = pulse->sData[j] - pulse->sData[j - kk] - pulse->sData[j - 1] +
               pulse->sData[j - kk - 1];
    ...
    case 3: { // CFD
      ...
      for (Int_t j = 0; j < (Int_t)pulse->sData.size(); j++) {
        ...
        if (j - opt.sDrv[pulse->Chan] >= 0 &&
            j + delay < (int)pulse->sData.size())
          dd = pulse->CFD(j, opt.sDrv[pulse->Chan], delay, opt.FF[pulse->Chan],
                          drv);
    ...
    if (fChn_on(ch)) {
      for (auto j = 0; j < Gr[dr][i]->GetN(); j++) {
        if (Gr[dr][i]->GetX()[j] >= x1 && Gr[dr][i]->GetX()[j] <= x2) {
          if (Gr[dr][i]->GetY()[j] < my1[dr])
            my1[dr] = Gr[dr][i]->GetY()[j];
          if (Gr[dr][i]->GetY()[j] > my2[dr])
            my2[dr] = Gr[dr][i]->GetY()[j];
        }
      }
...
void EventFrame::DrawPeaks(int dr, int j, PulseClass *pulse, double y1,
                           double y2) {
  ...
  if (fChn_on(ch)) {
    ...
    if (bthr && opt.b_peak[8]) { // threshold
      doYline(opt.sThr[pulse->Chan], gx1[dr][j], gx2[dr][j], chcol[pulse->Chan],
              2);
      doYline(opt.sLT[pulse->Chan], gx1[dr][j], gx2[dr][j], chcol[pulse->Chan],
              3);
    }
    ...
    if (pulse->Pos <= -32222) {
      // prnt("ssd d ls;",BRED,"ErrPos:
      // ",pulse->Pos,pulse->Chan,pulse->Tstamp64,RST);
      return;
    }
    ...
    if (opt.b_peak[1]) { // Pos
      // prnt("ss d d f f
      // fs;",KRED,"Pos:",dr,pulse->Pos,pulse->Pos+dt,y1,y2-dy*0.4,RST);
      doXline(pulse->Pos + dt, y1, y2 - dy * 0.4, 2, 1);
    }
    if (opt.b_peak[2]) { // Time
      // prnt("ss l d d f
      // fs;",BRED,"Pk:",pulse->Tstamp64,pulse->Chan,pulse->Pos,pulse->Time,dt,RST);
    ...
    if (dr == 0 && hcl->b_base[ch]) {
      ...
      // prnt("ss f f f f f fs;",BRED,"LBase:
      // ",B1+dt,B2+dt,Y1,Y2,pulse->Base,pulse->Sl1,RST);
      ...
      // prnt("ss f f f f f fs;",BGRN,"LPeak:
      // ",P1+dt,P2+dt,Y1,Y2,A0,pulse->Sl2,RST);
    }
  ...
  for (int kk = -1; kk < 31; kk++) {
    ...
    for (UInt_t j = 0; j < d_event->pulses.size(); j++) {
      ...
      if (fChn_on(pulse->Chan)) {
        ...
        // cout << "dt: " << j << " " << (int)pulse->Chan << " " << dt; //<<
        // endl; cout << " "<< Gr[i][j]->GetN() << " " << Gr[i][j]->GetX()[-dt]
        // << " " << Gr[i][j]->GetY()[-dt] << endl;
        ...
        if (nnn > 0) {
          TGraph *gg =
              new TGraph(nnn, Gr[i][j]->GetX() + xmin, Gr[i][j]->GetY() + xmin);
          ...
          // if (kk==1) {
          //   cout << "sum/N: " << d_event->Nevt << " " << j << " " <<
          //   (int)pulse->Chan << " " << kk << " " << sum/gg->GetN() << endl;
          // }
        }

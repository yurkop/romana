...
void Decoder::Decode_Start(Long64_t r_size, Long64_t o_size, bool b_acq, int module) {
  // r_size - nominal size of buffer (на самом деле + o_size)
  // o_size - offset size (offset в начале - перед буфером)
...
void Decoder::Copy_Start(bool b_acq) {
  if (b_acq) {
    copy_running = true;
...
void Decoder::Process_Start(int num_threads) {
  ...
  write_ptr = Buf_ring.b1;
  worker_ptrs.clear();
  for (UInt_t i = 0; i < num_process_threads; ++i) {
    process_threads.push_back(std::make_unique<std::thread>([this, i]() { 
      Process_Worker(i);
    }));
...
void Decoder::Copy_Worker() {
  ...
  while (true) {
    std::list<BufClass> local_queue;
    {
      ...
      copy_cond.wait(lock, [this]() {
	return !copy_queue.empty() || !copy_running;
      });
      if (!copy_running && copy_queue.empty()) break;
    ...
    try {
      ...
      UChar_t* pos=write_ptr.load(std::memory_order_acquire);//атомарный доступ

      for (auto& item : local_queue) {
	size_t data_size = item.b3 - item.b1;
	UChar_t* safe_limit = get_safe_write_limit();
	UChar_t* write_start = write_ptr.load(std::memory_order_acquire);

	if (!first_call) {
	  // Вычисляем доступное место до safe_limit
	  size_t available_space;
	  if (safe_limit >= write_start) {
	    available_space = safe_limit-write_start;
	  } else {
	    available_space = (safe_limit-Buf_ring.b1)+(Buf_ring.b3-write_start);
	  }

	  // Проверяем поместится ли data_size
	...
	// Обновляем atomic указатель
	write_ptr.store(Buf_ring.u82.b, std::memory_order_release);
      }

      Send_for_Process(BufProc.u82.b,pos,Buf_ring.u82);
...
void Decoder::
Send_for_Process(UChar_t* p1, UChar_t* p2, union82 &p3) {
  // проверить гонку с записью/другими потоками анализа

  BufClass pbuf;
  // p1=BufProc.u82.b  - конец предыдущего анализа
  ...
  if (p3.b<pbuf.b1) { //конец раньше, чем начало
    prnt("sss x xs;",BRED,"Invalid range p3.b < pbuf.b1:",BCYN,p3.b,p1,RST);
    BufProc.u82.b = p3.b;  // ← СБРАСЫВАЕМ на текущий конец
    return;
  }
...
void Decoder::Process_Worker(UInt_t thread_id) {
  ...
  while (process_running) {
    ...
    try {
      ...
      prnt("ss d l s x s x s ls;", BCYN, "Thread/buf", thread_id, Buf.bufnum,
	   "analyzing from:", Buf.b1, 
	   "to:", Buf.b3, "Bufevents:", Bufevents.size(), RST);
    }
    catch (const std::exception& e) {
...
UChar_t* Decoder::get_safe_write_limit() {
    UChar_t* current_write = write_ptr.load(std::memory_order_acquire);
    //size_t buffer_size = Buf_ring.b3 - Buf_ring.b1;
    size_t min_distance = SIZE_MAX;
    UChar_t* closest = nullptr;
    ...
    for (auto& worker_ptr : worker_ptrs) {
        ...
        if (ptr >= current_write) {
            distance = ptr - current_write;  // прямое расстояние
        } else {
            distance = (ptr - Buf_ring.b1) + (Buf_ring.b3 - current_write);  // через конец
        }
    ...
    return closest ? closest : Buf_ring.b1;
}
...
UChar_t* Decoder::FindEvent_backward_1(union82 From, UChar_t *To) {
  while (From.b > To) {
    From.us--;
    if (IsEventStart_1(From)) return From.b;
  }
  return To;
}
UChar_t* Decoder::FindEvent_backward_3(union82 From, UChar_t *To) {
  while (From.b > To) {
    From.us--;
    if (IsEventStart_3(From)) return From.b;
  }
  return To;
}
UChar_t* Decoder::FindEvent_backward_22(union82 From, UChar_t *To) {
  while (From.b > To) {
    From.us--;
    if (IsEventStart_22(From)) return From.b;
  }
  return To;
}
UChar_t* Decoder::FindEvent_backward_36(union82 From, UChar_t *To) {
  while (From.b > To) {
    From.ul--;
    if (IsEventStart_36(From)) return From.b;
  }
  return To;
}
UChar_t* Decoder::FindEvent_backward_79(union82 From, UChar_t *To) {
  while (From.b > To) {
    From.ul--;
    if (IsEventStart_79(From)) return From.b;
  }
  return To;
}
...
UChar_t* Decoder::FindEvent_backward(union82 From, UChar_t *To) {
  ...
  // std::call_once(init_all_flag,init_all_tables); //Один вызов для всех методов
...
void Decoder::Dec_Init(eventlist* &Blist, BufClass& Buf) {
  std::lock_guard<std::mutex> lock(buf_mutex);

  Bufevents.emplace_back(LocalBuf());
  Blist = &Bufevents.back().B;
  Bufevents.back().num = Buf.bufnum;
...
void Decoder::Decode_switch(BufClass& Buf) {
  ...
  // Быстрая диспетчеризация
  if (crs_module >= 0 && crs_module < MAX_HASH
      && decode_table[crs_module]) {
      (this->*decode_table[crs_module])(Buf);
      return;
  }
...
void Decoder::Decode22(BufClass& Buf) {
  // xxxx- нет!!!-> декодирует один импульс из буфера Buf, записывает его в pls
  ...
  while (uu.b+1<Buf.b3) {
    ...
    if (frmt==0) {
      ...
      if (ipls.ptype==0) {
	crs->PulseAna(ipls);
	crs->Event_Insert_Pulse(Blist,&ipls);
      }
...
void Decoder::Decode36(BufClass& Buf) {
  // xxxx- нет!!!-> декодирует один импульс из буфера Buf, записывает его в pls
  ...
  while (uu.b+7<Buf.b3) {
    ...
    case 11: { //Counters
      ...
      if (dt) {
	crs->rate_hard[ipls.Chan] = (ipls.Counter - crs->npulses3o[ipls.Chan])/dt;
      }
...
void Decoder::Event_Insert_Pulse(eventlist &Elist, PulseClass &pls) {
  ...
  */
  ...
  for (auto rit=Elist.rbegin();rit!=Elist.rend();++rit,++nn) {
    ...
    if (dt > opt.tgate) {
      //pls пришел позже, чем tgate -> добавляем новое событие ПОСЛЕ текущего
      auto it=Elist.emplace(rit.base(),EventClass());
      it->Nevt=crs->nevents;
      it->AddPulse(&pls);
      ++crs->nevents;
    ...
    // else: dt < -opt.tgate - импульс пришел раньше, продолжаем поиск к началу списка
  }

  // дошли до начала списка; вставляем новый event в начало (перед первым событием)
